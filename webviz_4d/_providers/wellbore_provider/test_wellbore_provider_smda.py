...
def main():
    ...
    print("\nGet planned wells for a selected field ...")
    start = time.time()
    planned_wellbores = pozo_provider.get_planned_wellbores(
        field_name=field,
    )
    end = time.time()
    print("Get planned wells:", end - start)
    if planned_wellbores:
        print(planned_wellbores.metadata.dataframe)
        print(planned_wellbores.trajectories.dataframe)
