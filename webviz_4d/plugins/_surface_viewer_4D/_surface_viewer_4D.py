...
class SurfaceViewer4D(WebvizPluginABC):
    ...
    def __init__(
        ...
        self.selector = SurfaceSelector(
            app, self.surface_metadata, self.map_defaults[0]
        )
        self.selector2 = SurfaceSelector(
            app, self.surface_metadata, self.map_defaults[1]
        )
        self.selector3 = SurfaceSelector(
            app, self.surface_metadata, self.map_defaults[2]
        )
        ...
        if os.path.isfile(surface_file):
            ...
            if self.zone_polygon_layers and len(self.zone_polygon_layers) > 0:
                for index, zone_polygon in enumerate(self.zone_polygon_layers):
                    ...
                    if (
                        layer is not None and len(layer) == 0
                    ):  # Specific polygon not found, use default
