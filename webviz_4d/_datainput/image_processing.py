...
def get_colormap(colormap):
    """Get selected colormap from matlplotlib, but modify inferno if selected"""

    if colormap == "inferno":
        cmap = [change_inferno()]
    else:
        cmap = cm.get_cmap(colormap, 256)([np.linspace(0, 1, 256)])

    return array_to_png(cmap, colormap=True)
