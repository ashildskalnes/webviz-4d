    ...
    for attribute_object in attribute_objects:
        ...
        if metadata_version is None:
            ...
            if tags.get("Source.* Horizon") != name:
                ...
                seismic_horizon_objects = osdu_service.get_seismic_horizons(
                    seismic_names
                )
                ...
                if selected_metadata_version == "0.3.1":
                    base_seismic_name = tags.get(
                        "SeismicProcessingTraces.BaseSeismicTraces"
                    )
                    monitor_seismic_name = tags.get(
                        "SeismicProcessingTraces.MonitorSeismicTraces"
                    )
                ...
                for seismic_object in seismic_objects:
                    ...
                    print(
                        "        - Begin date:",
                        datetime.strftime(begin_date, "%Y-%m-%d"),
                    )
                    print(
                        "        - End date:", datetime.strftime(end_date, "%Y-%m-%d")
                    )
                    print(
                        "        - Reference date:",
                        datetime.strftime(reference_date, "%Y-%m-%d"),
                    )
